...
describe("real supplier data policy", () => {
  it("allows simulated supplier data only inside automated tests", () => {
    ...
    // 部署环境即使显式开启旧模拟开关，也不允许返回假数据。
    expect(simulatedSupplierDataAllowed("production", "sandbox", "true")).toBe(false);
  ...
  it("uses local hotels only for mock mode or isolated automated tests", () => {
    expect(localHotelSimulationAllowed("mock", false, "production", "false")).toBe(true);
    expect(localHotelSimulationAllowed("sandbox", false, "production", "true")).toBe(false);
    expect(localHotelSimulationAllowed("sandbox", false, "test", "false")).toBe(true);
