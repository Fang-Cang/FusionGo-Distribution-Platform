...
> 当前阻塞（2026-08-06）：应用尚未接入真实企业身份。`AUTH_MODE=external` 只是生产配置要求，不代表认证已经实现；当前生产模式中的用户接口会拒绝未建立可信服务端会话的请求。运维仅配置域名或反向代理不能关闭此问题，必须由应用接入 OIDC/SSO、服务端会话、租户和角色映射后再发布。
...
4. 外部身份系统已保护整个站点和 API，用户、租户、角色可追溯。
   本地/Sandbox 的邮箱密码自助注册不得作为生产账号体系；生产账号开通、停用、密码、MFA 与角色必须由企业身份系统管理。
...
14. 使用已确认酒店订单与已出票机票订单分别下载 PDF，核对中文字体、一页 A4 布局、订单号、供应商单号、金额、人员脱敏信息与缺失字段提示。
